...
import { GetMobileLogRequest, GetMobileLogResponse, MobileLogs } from "types/cdu-requests";
import { v4 as uuidv4 } from "uuid";

export default function handler(
  req: NextApiRequest,
  res: NextApiResponse<Partial<GetMobileLogResponse>>
) {
  if (req.method !== "GET")
    return res.status(405).json({ message: "method not allowed", status: "failed" });

  const { no_of_records, page_no, email } = req.query as unknown as GetMobileLogRequest;
